                ...
                <div class="col-lg-9">
                    ...
                    <div class="card border-0 shadow mb-4">
                        <div class="card-body p-4">
                            <h3 class="fs-4 mb-4">Career Objective</h3>

                            {{-- Display existing Objective --}}
                            @if(!empty($objective))
                                <div class="mb-4">
                                    <h5 class="mb-2">Your Objective:</h5>
                                    <p id="objectiveText">{{ $objective->objective }}</p>
                                </div>
                            @else
                                <p class="text-muted">No career objective added yet.</p>
                            @endif
                            ...
                            {{-- Hidden Edit Form --}}
                            <form action="{{ asset('/postJob.store') }}" method="POST" id="editForm" style="display: none; margin-top: 30px;">
                                @csrf
                                <input type="hidden" name="user_id" value="{{ Auth::id() }}">
                                <div class="mb-4">
                                    <label for="objective" class="form-label">Update Objective</label>
                                    <textarea name="objective" id="objective" class="form-control" rows="4" required>{{ $objective->objective ?? '' }}</textarea>
                                </div>
