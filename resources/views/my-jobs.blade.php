            ...
            <div class="row">
                ...
                <div class="col-lg-9">
                    ...
                    <div class="card border-0 shadow p-4">
                        <h4 class="mb-4"><b>My Posted Jobs</b></h4>
                            ...
                            <table class="table table-striped table-hover align-middle">
                                ...
                                <tbody>
                                    @foreach ($jobs as $job)
                                        ...
                                        <tr class="job-row">
                                            <td>{{ $job->id ?? 'N/A' }}</td>
                                            <td>{{ $jobTitle->job_title ?? 'No Job Title' }}</td>
                                            ...
                                            <td>
                                                ...
                                                @endphp
                                                <span class="badge {{ $statusClass }}">{{ ucfirst($job->status ?? 'Unknown') }}</span>
                                            </td>
                                            <td>
                                                @if($job->payment)
                                            ...
                                            <td>
                                                <div class="dropdown">
                                                    ...
                                                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton{{ $job->id }}">
                                                        <li><a class="dropdown-item" href="{{ route('job-details', ['id' => $job->id]) }}">View</a></li>
                                                        @if(\Carbon\Carbon::parse($job->expire_date)->lt(\Carbon\Carbon::now()))
                                                            <li>
                                                                <form action="{{ route('jobs-destroy', ['id' => $job->id]) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this job?');">
                                                                    @csrf
                                                                    @method('DELETE')
                                                            ...
                                                            @if($job->payment)
                                                        @elseif($job->payment->payment_status == 'failed')
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            function updateJobCount(total) {
                jobCountDisplay.innerText = `Total Jobs Posted: ${total}`;
            }
